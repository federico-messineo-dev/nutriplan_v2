  ...
  const handleGenerate = async () => {
    ...
    try {
      const r = await fetchWithTimeout("/api/plans/generate", {
        ...
        body: JSON.stringify({ clientId: genClientId, type: "diet" }),
      });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || "Errore");
      setGenMessage("Piano generato con successo! Ricarica la pagina per vederlo.");
      fetchPlans();
    } catch (err) {
        ...
        <main className="flex-1 overflow-auto p-4 md:p-6 mobile-bottom-pad">
          <div className="max-w-6xl mx-auto">
            ...
            >
              ...
              >
                <Sparkles size={12} />
                {generating ? "Generando..." : "Genera piano AI"}
              </button>
              {genMessage && (
