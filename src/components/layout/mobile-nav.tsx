...
export function MobileNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-slate-900/90 border-t border-slate-700/50 backdrop-blur-lg safe-area-bottom">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "relative flex flex-col items-center justify-center gap-0.5 w-14 h-14 rounded-lg transition-colors",
                isActive
                  ? "text-cyan-400"
            ...
            >
              ...
              <span className="relative z-10 text-[10px] font-body leading-none">{item.label}</span>
            </Link>
          );
        })}
      </div>
